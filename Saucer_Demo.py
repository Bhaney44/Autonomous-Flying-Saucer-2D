...
# Firing mainengine is -0.3 points each frame.
...
LANDER_POLY =[(-10,+17), (-27,10), (-17,-10),(+17,-10), (+27,10), (+10,+17)]
...
class LunarLander(gym.Env, EzPickle):
    ...
    def reset(self):
        ...
        self.moon.color1 = (2.0,1.0,2.0)
        self.moon.color2 = (2.0,1.0,2.0)
        initial_y = VIEWPORT_H/SCALE

        #Lander
        self.lander = self.world.CreateDynamicBody(position = (VIEWPORT_W/SCALE/2, initial_y),angle=0.0,
            fixtures = fixtureDef(shape=polygonShape(vertices=[ (x/SCALE,y/SCALE) for x,y in LANDER_POLY ]),
            density = 5.0,friction=0.1,categoryBits=0x0010, maskBits=0x001,restitution=0.0))

        #Lander Color
        self.lander.color1 = (1.0,0.0,1.0)
        self.lander.color2 = (1.0,0.0,1.0)
        ...
        for i in [-1,+1]:
            leg = self.world.CreateDynamicBody(
                ...
                angle = (i*0.05),
                fixtures = fixtureDef(shape=polygonShape(box=(LEG_W/SCALE, LEG_H/SCALE)),density=1.0,
                    restitution=0.0,categoryBits=0x0020, maskBits=0x001))
            
            leg.ground_contact = False
            leg.color1 = (1.0,1.0,1.0)
            leg.color2 = (1.0,1.0,1.0)
            rjd = revoluteJointDef(bodyA=self.lander, bodyB=leg, localAnchorA=(0, 0),
                localAnchorB=(i*LEG_AWAY/SCALE, LEG_DOWN/SCALE), enableMotor=True, enableLimit=True,
                maxMotorTorque=LEG_SPRING_TORQUE, motorSpeed=+0.3*i)
            if i==-1:
                #Angles legs have freedom to travel within
    ...
    def _create_particle(self, mass, x, y, ttl):
        p = self.world.CreateDynamicBody(position = (x,y), angle=0.0,fixtures = fixtureDef(shape=circleShape(radius=2/SCALE, pos=(0,0)),
    ...
    def step(self, action):
        ...
        state = [(pos.x - VIEWPORT_W/SCALE/2) / (VIEWPORT_W/SCALE/2),
            ...
            vel.x*(VIEWPORT_W/SCALE/2)/FPS, vel.y*(VIEWPORT_H/SCALE/2)/FPS,
            self.lander.angle, 20.0*self.lander.angularVelocity/FPS,
            1.0 if self.legs[0].ground_contact else 0.0, 1.0 if self.legs[1].ground_contact else 0.0]
        assert len(state)==8
    ...
    def render(self, mode='human'):
        ...
        for obj in self.particles:
            obj.ttl -= 0.15
            obj.color1 = (0.0,1.0,1.0)
            obj.color2 = (0.0,1.0,1.0)

        self._clean_particles(False)

        for p in self.sky_polys:
            #Sky color
            self.viewer.draw_polygon(p, color=(0.0,1.0,1.0))
        ...
        for x in [self.helipad_x1, self.helipad_x2]:
            ...
            #Flag color
            self.viewer.draw_polyline( [(x, flagy1), (x, flagy2)], color=(1.0,0.0,1.0))
            self.viewer.draw_polygon( [(x, flagy2), (x, flagy2-10/SCALE), (x+25/SCALE, flagy2-5/SCALE)], color=(1.0,0.0,1.0))
